...
export default function FolderCard({ to, onClick, title, cover, count, subtitle, testId }: Props) {
  const inner = (
    <>
      {/* folder tab */}
      <span className="absolute -top-2 left-5 z-10 h-4 w-20 rounded-t-lg border border-b-0 border-[var(--line)] bg-[#2E1118]" />

      <div className="relative overflow-hidden rounded-2xl border border-[var(--line)] bg-[#230C12] p-2.5 shadow-[0_18px_40px_-24px_rgba(0,0,0,0.9)] transition-all duration-300 group-hover:-translate-y-1.5 group-hover:border-[var(--gold)]/45">
        <div className="folder-sheen relative aspect-square overflow-hidden rounded-xl bg-[#2E1118]">
          {cover ? (
            <img
              src={cover}
              alt={title}
          ...
          <span className="absolute right-2 top-2 inline-flex items-center gap-1 rounded-full border border-[var(--gold)]/35 bg-[#150609]/80 px-2 py-0.5 text-[10px] font-medium text-[var(--gold-soft)] backdrop-blur-sm">
