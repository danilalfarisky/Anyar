...
export default function PhotoManagerModal({ open, onOpenChange, client, onSaved }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[88vh] w-[min(95vw,64rem)] !max-w-none overflow-hidden border-[#E7DFD5] bg-white">
        <DialogHeader>
          ...
          {detail.isPending ? (
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="aspect-[4/3] animate-pulse rounded-xl bg-[#F5EFEB]" />
              ))}
            </div>
          ) : order.length === 0 ? (
              ...
              {order.map((photo, i) => {
                ...
                return (
                  <div
                    ...
                    className="overflow-hidden rounded-xl border border-[#E7DFD5] bg-white"
                  >
                    <div className="relative aspect-[4/3] overflow-hidden bg-[#F5EFEB]">
                      ...
                      <span className="absolute left-2 top-2 rounded-full bg-[#1C1917]/75 px-2 py-0.5 text-xs font-medium text-white">
                        {i + 1}
                      </span>
                      {isCover && (
                        <span
                          data-testid="photo-manager-cover-badge"
                          className="absolute right-2 top-2 rounded-full bg-[#FEF3C7] px-2 py-0.5 text-[10px] font-semibold text-[#78350F]"
                        >
                          SAMPUL
                        </span>
                      )}
